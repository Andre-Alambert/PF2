...
def main():
    ...
    for cfg_idx, params in enumerate(PARAM_GRID):
        ...
        print(f"[{cfg_idx + 1:>2}/{n_configs}] "
              f"pop={params['pop_size']:>2}  gen={params['num_gen']:>2}  "
    ...
    csv_path = results_dir / f"experiment_nsga2_{case_name}_{timestamp}.csv"
    ...
    # ── Salvar gráfico ───────────────────────────────────────────────────────
    plot_path = results_dir / f"experiment_nsga2_{case_name}_{timestamp}.png"
    plot_hypervolume_pareto(agg_rows, plot_path, algo_label="NSGA-II")
